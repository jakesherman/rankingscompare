...
def same_tau_a_values():
    ...
    for test_case in [func() for func in [generate_test_case] * 100]:
        mykt, scipykt = rc.tau_a(*test_case), kendalltau(*test_case)[0]
...
class TauTestCase(unittest.TestCase):
    ...
    def test_tau_a_4(self):
        self.assertRaises(AssertionError, rc.tau_a, self.a, self.e)

    # Should raise an AssertionError for non-unique lists
    def test_tau_a_5(self):
        self.assertRaises(AssertionError, rc.tau_a, [1, 1, 2], [1, 2, 2])

    # Agreement between myself and scipy
    def test_same_tau_a_values(self):
        self.assertTrue(same_tau_a_values())
